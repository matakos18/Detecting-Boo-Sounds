...
def plotSegmentationResults(flags_ind, flags_ind_gt, class_names, mt_step, ONLY_EVALUATE=False):
    ...
    segs_classes = flags2segs(flags, mt_step)
    print(segs_classes)
    (segs, classes)=segs_classes
...
#def trainHMM_fromDir(dirPath, hmm_model_name, mt_win, mt_step, st_win, st_step):
def trainHMM_fromDir(dirPath, hmm_model_name, st_win, st_step):
    '''
    ...
    for i, f in enumerate(glob.glob(dirPath + os.sep + '*.wav')):
        # for each WAV file
        wav_file = f
        print("processing file: "+str(wav_file))
        gt_file = f.replace('.wav', '.segments')
        if not os.path.isfile(gt_file):
            continue
        [seg_start, seg_end, seg_labs] = readSegmentGT(gt_file)
#        flags, class_names = segs2flags(seg_start, seg_end, seg_labs, mt_step)
        flags, class_names = segs2flags(seg_start, seg_end, seg_labs, st_step)
...
#        [F, _, _] = aF.mtFeatureExtraction(x, fs, mt_win * fs, mt_step * fs, round(fs * st_win), round(fs * st_step))
        x=audioBasicIO.stereo2mono(x)
        [F, _] = aF.stFeatureExtraction(x, fs, fs*st_win, fs*st_step)
...
#        mt_step = cPickle.load(fo)        
    ...
    x=audioBasicIO.stereo2mono(x)
    [Features,_]=aF.stFeatureExtraction(x, fs, fs*st_win, fs*st_step)
#    [Features, _, _] = aF.mtFeatureExtraction(x, fs, mt_win * fs, mt_step * fs,round(fs * 0.050),round(fs * 0.050))
...
#        flags_gt, class_names_gt = segs2flags(seg_start, seg_end, seg_labs, mt_step)
        flags_gt, class_names_gt = segs2flags(seg_start, seg_end, seg_labs, st_step)
        flagsGTNew = []
    ...
    print("flags_ind: "+str(flags_ind)) 
#    acc = plotSegmentationResults(flags_ind, flags_ind_gt, classes_all, mt_step, not plot_res)
    acc = plotSegmentationResults(flags_ind, flags_ind_gt, classes_all, st_step, not plot_res)
    if acc >= 0:
...
def apply_filter(st_feats, feat_number, sWindow, weigh=0.5, filter_type="upper"):

    # Step 2: train binary svm classifier of low vs high energy frames
    # keep only the energy short-term sequence (2nd feature)
    st_feat = st_feats[feat_number, :]
    en = numpy.sort(st_feat)
    ...
    if filter_type=="upper":
        class1 = st_feats[:, numpy.where( st_feat <= t1)[0]]
        # get all features that correspond to high energy
        class2 = st_feats[:, numpy.where( st_feat >= t2)[0]]
    elif filter_type=="lower":
        class1 = st_feats[:, numpy.where( st_feat >= t2)[0]]
        # get all features that correspond to high energy
        class2 = st_feats[:, numpy.where( st_feat <= t1)[0]]
    else:
    ...
    # (ONSET vs SILENCE)
    class1_len=numpy.shape(class1.T)[0]
    class2_len=numpy.shape(class2.T)[0]
    print("shape of class1: "+str(class1_len))
    print("shape of class2: "+str(class2_len))
    ...
    for i in range(st_feats.shape[1]):
        ...
        probability=svm.predict_proba(cur_fv.reshape(1,-1))[0][1]
        prob_on_set.append(probability)
 #       mfcc9_prob_on_set.append(mfcc9_svm.predict_proba(mfcc9_cur_fv.reshape(1,-1))[0][1])
    prob_on_set = numpy.array(prob_on_set)
 #   mfcc9_prob_on_set=numpy.array(mfcc9_prob_on_set)
    # smooth probability:
    prob_on_set = smoothMovingAvg(prob_on_set, sWindow)
    ...
    if class1_len > 20 and class2_len >20 : 
        T = (numpy.mean((1 - weigh) * prog_on_set_sort[0:Nt]) + weigh * numpy.mean(prog_on_set_sort[-Nt::]))
    else:
        T=0
    max_idx = []
    for ind in numpy.where(prob_on_set > T)[0] :
        max_idx.append(ind) 
        
        
    return max_idx
    
def silenceRemoval3(x, fs, st_win, st_step, smoothWindow=0.5, plot=False):


    # Step 1: feature extraction
    x = audioBasicIO.stereo2mono(x)

    st_feats, _ = aF.stFeatureExtraction(x, fs, round(st_win * fs), round(st_step * fs))
    
    # Step 2: train binary svm classifier of low vs high energy frames
    # get the indices of the frames that satisfy the thresholding
    
    max_idx=apply_filter(st_feats, 1, smoothWindow/st_step, 0.80, "upper")
    
    
    

#    new_feats=st_feats[:,max_idx]
#    new_idx=apply_filter(new_feats , 9, smoothWindow/st_step, 0.90, "lower")
#    max_idx=[max_idx[i] for i in new_idx]    
...
#    new_feats=st_feats[:,max_idx]
#    new_idx=apply_filter(new_feats , 14, smoothWindow/st_step, 0.80, "lower")
#    max_idx=[max_idx[i] for i in new_idx]
...
#            if max_idx[i+1]*st_step - cur_cluster[0]*st_step > 3.0:
#                break
        ...
        seg_limits.append([round(cur_cluster[0] * st_step,1),
                           round(st_win+ cur_cluster[-1] * st_step,1)])
...
#    new_feats=st_feats[:,max_idx]
#    new_idx=apply_filter(new_feats, 7, smoothWindow/ st_step, 0.20, "upper")
#    max_idx=[max_idx[i] for i in new_idx]
...
#    new_feats=st_feats[:,max_idx]
#    new_idx=apply_filter(new_feats, 11, smoothWindow/ st_step, 0.40, "upper")
#    max_idx=[max_idx[i] for i in new_idx]
...
def train_classifier_feature(st_feats, fea, typ="upper"):

    # Step 2: train binary svm classifier of low vs high energy frames
    # keep only the energy short-term sequence (2nd feature)
    st_feat = st_feats[fea, :]
    en = numpy.sort(st_feat)
    # number of 10% of the total short-term windows
    l1 = int(len(en) / 10)
    # compute "lower" 10% energy threshold
...
def mfcc_silenceRemoval(st_feats, st_win, st_step, smoothWindow=0.5):
...
#    mfcc2_svm, mfcc2_means_s, mfcc2_stds_s=train_classifier_feature(9,"lower")    


    svm, means_s, stds_s=train_classifier_feature(st_feats, 2,"upper")

#    svm_2, means_s_2, stds_s_2=train_classifier_feature(st_feats, 2,"upper")
    

#    mfcc3_svm, mfcc3_means_s, mfcc3_stds_s=train_classifier_feature(10,"lower")
 ...
 #   mfcc9_prob_on_set=numpy.array(mfcc9_prob_on_set)
    ...
    # of top 10% and lower 10% of the values
    Nt = int(prog_on_set_sort.shape[0] / 10)
    T = (numpy.mean((1 - weight) * prog_on_set_sort[0:Nt]) + weight * numpy.mean(prog_on_set_sort[-Nt::]))

    max_idx = []
    for ind in numpy.where(prob_on_set > T)[0] :
        max_idx.append(ind)
#        mfcc9_cur_fv=(st_feats[:, ind] - means_s_2) / stds_s_2

#        if svm_2.predict_proba(mfcc9_cur_fv.reshape(1,-1))[0][1] >= 0.30:
#                max_idx.append(ind)



    new_feats=st_feats[:,max_idx]
#    filter1=apply_filter(new_feats, 0, smoothWindow/ st_step, 0.70, "upper")
#    filter2=apply_filter(new_feats, 5, smoothWindow/st_step, 0.30, "lower")
#    filter7=apply_filter(new_feats, 8, smoothWindow/st_step, 0.20, "upper")

    #the following were removed for normalized features
#    filter4=apply_filter(new_feats, 9, smoothWindow/st_step, 0.30, "lower")
    filter10=apply_filter(new_feats, 18, smoothWindow/st_step, 0.30, "lower")


    filter9=apply_filter(new_feats, 14, smoothWindow/st_step, 0.30, "lower")
#    filter11=apply_filter(new_feats, 16, smoothWindow/st_step, 0.50, "upper")


    #this is a new filter. Needs to be tested
    filter13=apply_filter(new_feats, 15, smoothWindow/st_step, 0.30, "upper")
...
#    filter6=apply_filter(new_feats, 11, smoothWindow/st_step, 0, "upper")
#    filter5=apply_filter(new_feats, 10, smoothWindow/st_step, 0.30, "lower")
    #this was removed for the werder-bayern game. Try with normalized:
#    filter12=apply_filter(new_feats, 17, smoothWindow/st_step, 0.50, "lower")


    
    
    set1=set(filter10).intersection(set(filter9))#.intersection(set(filter4))#.intersection(set(filter2))
    filtered=list(set1.intersection(set(filter13)))#.intersection(set(filter10)))
    filtered.sort()
    max_idx=[max_idx[i] for i in filtered]
...
#                break
        ...
        seg_limits.append([round(cur_cluster[0] * st_step,1),
                           round(st_win+cur_cluster[-1] * st_step,1)])
        idx_limits.append([cur_cluster[0], cur_cluster[-1]])
...
#    seg_limits = seg_limits_2
    ...
    return seg_limits,idx_limits



def silenceRemoval(st_feats, st_win, st_step, smoothWindow=0.5):
...
#    zcr_svm, zcr_means_s, zcr_stds_s=train_classifier_feature(0,"lower")

    mfcc9_svm, mfcc9_means_s, mfcc9_stds_s=train_classifier_feature(st_feats, 16,"upper")
    

#    mfcc3_svm, mfcc3_means_s, mfcc3_stds_s=train_classifier_feature(10,"lower")

    svm,means_s,stds_s=train_classifier_feature(st_feats ,1,"upper")
 ...
 #   mfcc9_prob_on_set=numpy.array(mfcc9_prob_on_set)
    ...
    max_idx = []
    for ind in numpy.where(prob_on_set > T)[0] :
        mfcc9_cur_fv=(st_feats[:, ind] - mfcc9_means_s) / mfcc9_stds_s
#        mfcc3_cur_fv=(st_feats[:, ind] - mfcc3_means_s) / mfcc3_stds_s
#        mfcc2_cur_fv=(st_feats[:, ind] - mfcc2_means_s) / mfcc2_stds_s
#        zcr_cur_fv=(st_feats[:, ind] - zcr_means_s) / zcr_stds_s
        if mfcc9_svm.predict_proba(mfcc9_cur_fv.reshape(1,-1))[0][1] >= 0.90:
                max_idx.append(ind)
#            if mfcc3_svm.predict_proba(mfcc3_cur_fv.reshape(1,-1))[0][1] >= 0.90:
#                if zcr_svm.predict_proba(zcr_cur_fv.reshape(1,-1))[0][1] >= 0.70:
#                 if mfcc2_svm.predict_proba(mfcc2_cur_fv.reshape(1,-1))[0][1] >= 0.90:



    new_feats=st_feats[:,max_idx]
#    filter1=apply_filter(new_feats, 0, smoothWindow/ st_step, 0.70, "upper")
#    filter2=apply_filter(new_feats, 5, smoothWindow/st_step, 0.40, "upper")
    filter3=apply_filter(new_feats, 7, smoothWindow/st_step, 0.70, "upper")
#    filter7=apply_filter(new_feats, 8, smoothWindow/st_step, 0.20, "upper")

    #the following were removed for normalized features
    filter4=apply_filter(new_feats, 9, smoothWindow/st_step, 0.85, "lower")
#    filter10=apply_filter(new_feats, 18, smoothWindow/st_step, 0.70, "lower")


    filter9=apply_filter(new_feats, 14, smoothWindow/st_step, 0.85, "lower")
#    filter11=apply_filter(new_feats, 16, smoothWindow/st_step, 0.50, "upper")


    #this is a new filter. Needs to be tested
#    filter13=apply_filter(new_feats, 15, smoothWindow/st_step, 0.75, "upper")
...
#    filter6=apply_filter(new_feats, 11, smoothWindow/st_step, 0, "upper")
    filter5=apply_filter(new_feats, 10, smoothWindow/st_step, 0.70, "lower")
    #this was removed for the werder-bayern game. Try with normalized:
#    filter12=apply_filter(new_feats, 17, smoothWindow/st_step, 0.20, "lower")


    
    
    set1=set(filter5).intersection(set(filter9)).intersection(set(filter4))#.intersection(set(filter1))
    filtered=list(set1.intersection(set(filter3)))#.intersection(set(filter13)))#.intersection(set(filter4)))
    filtered.sort()
    max_idx=[max_idx[i] for i in filtered]
