...
def stfeatureAndTrain(list_of_dirs, st_win, st_step, classifier_type, model_name, compute_beat=False, perTrain=0.90):


#def featureAndTrain(list_of_dirs, mt_win, mt_step, st_win, st_step,classifier_type, model_name,compute_beat=False, perTrain=0.90):
    '''
...
#    [features, classNames, _] = aF.dirsWavFeatureExtraction(list_of_dirs,mt_win,mt_step,st_win, st_step, compute_beat=compute_beat)
    
    [features, classNames, _] = aF.dirsFeatureLoad(list_of_dirs)
#    [features, classNames, _] = aF.stdirsWavFeatureExtraction(list_of_dirs,st_win,st_step)
...
#        mt_step = cPickle.load(fo)
    ...
    return(SVM, MEAN, STD, classNames, st_win, st_step, compute_beat)
...
def smoother(inputSignal, windowLen=11):
...
def featfileClassification(feats, model_name, model_type, smoothWindow, prob_thresh):
    # Load classifier:
...
#        return (-1, -1, -1)
    ...
    if model_type == 'knn':
        ...
        [classifier, MEAN, STD, classNames, st_win, st_step, compute_beat] = stload_model(model_name)
...
#    [Fs, x]=fsx
    ...
    # feature extraction:
#    [st_feats, s] = aF.stFeatureExtraction(x, Fs, Fs * st_win, Fs * st_step)
#    [st_feats, s] = aF.stFeatureExtraction(x, Fs, Fs * st_win, Fs * st_step)

#    [faets_s_norm, means_s, stds_s] = normalizeFeatures([st_feats])
#    means_s=st_feats.mean(axis=1)
#    stds_s=st_feats.std(axis=1)
#    st_features = st_features.mean(axis=1)        # long term averaging of mid-term statistics
    prob_on_set = []
    for i in range(feats.shape[1]):
        # for each frame
#        cur_fv =(( (st_feats[:,i]+st_feats[:,i+1]) /2) - MEAN )/STD
        cur_fv =(feats[:,i] - MEAN )/STD
        # get svm probability (that it belongs to the ONSET class)
        prob_on_set.append(classifier.predict_proba(cur_fv.reshape(1,-1))[0][0])
    prob_on_set = numpy.array(prob_on_set)

    # smooth probability:
    prob_on_set = smoother(prob_on_set, smoothWindow / st_step)
    
    
    max_idx = []
    for ind in numpy.where(prob_on_set > prob_thresh)[0]:
        max_idx.append(ind)
    i = 0
    time_clusters = []
    seg_limits = []
