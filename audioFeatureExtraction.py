...
def mfccExtraction(signal, fs, win, step, normalize=False): 
    win = int(win)
    step = int(step)
    ...
    n_mfcc_feats = 13
    n_delta_feats = 13
    n_total_feats = n_time_spectral_feats + n_mfcc_feats + n_harmonic_feats + n_delta_feats
#    n_total_feats = n_time_spectral_feats + n_mfcc_feats + n_harmonic_feats
    ...
    feature_names += ["delta_{0:d}".format(delta_i) 
                      for delta_i in range(1, n_delta_feats+1)]
    spectral_features = []
    mfcc_features= []
    while (cur_p + win - 1 < N):                        # for each short-term window until the end of signal
        ...
        curFV = numpy.zeros((n_time_spectral_feats, 1))
        ...
        mfcc_vector = stMFCC(X, fbank, n_mfcc_feats).copy()    # MFCCs
 
        spectral_features.append(curFV)
    ...
    mfcc_features = numpy.vstack(mfcc_features)
    spectral_features = numpy.concatenate(spectral_features, 1)
    delta_features=delta(mfcc_features,2).transpose()
    st_features=numpy.concatenate((spectral_features,mfcc_features.transpose(),delta_features))
    
        #normalization
    if normalize is True:
        for i in range(st_features.shape[0]):
            minimum=numpy.min(st_features[i,:])
            maximum=numpy.max(numpy.abs(st_features[i,:]))
            st_features[i,:]=(st_features[i,:]-minimum)/(maximum-minimum)
...
#    st_features, f_names = stFeatureExtraction(signal, fs, st_win, st_step)
    st_features, f_names= mfccExtraction(signal, fs, st_win, st_step)
...
def dirFeatureLoad(dirName):
    all_st_feats = numpy.array([])
    feat_file_list = []
    feat_file_list.extend(glob.glob(os.path.join(dirName, '*.feat')))
    feat_file_list = sorted(feat_file_list)    
    feat_file_list2, st_feature_names = [], []
    if len(feat_file_list)==0:
        print("Directory does not contain .feat files")
        return
    for i, featFile in enumerate(feat_file_list):
        feat_file_list2.append(featFile)
        [st_term_feats, st_feature_names] = pickle.load(open(featFile, "rb" ))
        st_term_feats = numpy.transpose(st_term_feats)
        st_term_feats = st_term_feats.mean(axis=0)
        # long term averaging of mid-term statistics
        if (not numpy.isnan(st_term_feats).any()) and \
                (not numpy.isinf(st_term_feats).any()):
            if len(all_st_feats) == 0:
                # append feature vector
                all_st_feats = st_term_feats
            else:
                all_st_feats = numpy.vstack((all_st_feats, st_term_feats))
        else:
            print("error: features are not numbers")
    return (all_st_feats, feat_file_list2, st_feature_names)

def stdirWavFeatureExtraction(dirName, st_win, st_step):
    all_st_feats = numpy.array([])
    process_times = []
    ...
    wav_file_list = sorted(wav_file_list)    
    wav_file_list2, st_feature_names = [], []
    for i, wavFile in enumerate(wav_file_list):        
        print("Analyzing file {0:d} of "
              "{1:d}: {2:s}".format(i+1,
                                    len(wav_file_list),
        ...
        [st_term_feats, st_feature_names] =stFeatureExtraction(x, fs, st_win * fs, st_step * fs)
        st_term_feats = numpy.transpose(st_term_feats)
        st_term_feats = st_term_feats.mean(axis=0)
        # long term averaging of mid-term statistics
        if (not numpy.isnan(st_term_feats).any()) and \
    ...
    return (all_st_feats, wav_file_list2, st_feature_names)


        
def dirsFeatureLoad(dirNames):
    features = []
    classNames = []
    fileNames = []
    for i, d in enumerate(dirNames):
        [f, fn, feature_names] = dirFeatureLoad(d)
        if f.shape[0] > 0:
            # if at least one audio file has been found in the provided folder:
            print("f shape: "+str(numpy.shape(f)))
            features.append(f)
            fileNames.append(fn)
            if d[-1] == os.sep:
...
def stdirsWavFeatureExtraction(dirNames, st_win, st_step):
    '''
    ...
    fileNames = []
    for i, d in enumerate(dirNames):
        [f, fn, feature_names] = stdirWavFeatureExtraction(d, st_win, st_step)
        if f.shape[0] > 0:
            # if at least one audio file has been found in the provided folder:
            print("f shape: "+str(numpy.shape(f)))
            features.append(f)
